...
heading = Label(frame, text='Sign Up', fg="#57a1f8", bg='white', font=('Microsoft Yahi UI Light', 23, 'bold'))
...
con_pass_code = Entry(frame, width=25, fg='black', border=0, bg='white', font=('Microsoft Yahi UI Light', 11))
con_pass_code.place(x=30,y=240)
con_pass_code.insert(0, 'Confirm Password')

def on_enter(e):
    con_pass_code.delete(0,'end')
def on_leave(e):
    if con_pass_code.get()=='':
        con_pass_code.insert(0, 'Confirm Password')

con_pass_code.bind("<FocusIn>", on_enter)
con_pass_code.bind("<FocusOut>", on_leave)

Frame(frame, width=295, height=2, bg='black').place(x=25,y=270)
#######-------------------------------------------------------------------

Button(frame, width=39, pady=7, text='Sign Up', bg='#57a1f8', fg='white', border=0,command=signup).place(x=35, y=290)
